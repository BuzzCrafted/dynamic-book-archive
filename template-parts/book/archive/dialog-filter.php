...
?>
<dialog id="book-archive-dialog-filter" class="relative z-50 hidden w-[min(100vw-2rem,26rem)] max-h-[min(92dvh,40rem)] gap-5 overflow-visible rounded-2xl border-0 bg-surface p-6 text-body shadow-main backdrop:bg-page/80 sm:p-7 open:m-0 open:flex open:max-h-[min(92dvh,40rem)] open:flex-col open:fixed open:inset-auto"
	x-ref="filterDialog"
	@close="onFilterClose()"
	...
	<div class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
		<div class="flex flex-col gap-5">
			<div class="flex flex-col">
				...
				<select id="book-archive-toolbar-author-staging" x-ref="stagingAuthor" class="mt-0 w-full appearance-auto rounded-lg bg-filters-background px-3 py-2.5! text-sm text-filters-text shadow-main! border-none! focus:outline-none focus-visible:ring-0 focus-visible:ring-filters-link/50"></select>
			</div>
			<div class="flex flex-col">
				...
				<select id="book-archive-toolbar-tag-staging" x-ref="stagingTag" class="mt-0 w-full appearance-auto rounded-lg bg-filters-background px-3 py-2.5! text-sm text-filters-text shadow-main! border-none! focus:outline-none focus-visible:ring-0 focus-visible:ring-filters-link/50"></select>
			</div>
			<div class="flex flex-col">
				<?php
				get_template_part(
